...
def get_db_ganji_for_query(query_text):
    ...
    try:
        ...
        if not res_json.get('found', False):
            return ""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            if 'lifetime_script' in st.session_state:
                ...
                if prompt:
                    ...
                    # '오늘'의 간지 데이터 강제 조회 (질문과 상관없이 항상 조회)
                    ...
                    if today_row:
                        ...
                        [📅 오늘 날짜 기준 시스템 DB 데이터]
                        - 날짜: {cur_date_str}
                        - 간지: {today_row[2]}(년) {today_row[3]}(월) {today_row[4]}(일)
                        - 참고: 사용자가 특정 날짜를 묻지 않았다면, 이 데이터를 기준으로 '오늘'의 운세를 설명하거나, 
                               "오늘({today_row[4]})은 ~한 날이니 ~하는 게 좋습니다"라고 응용하여 답변하십시오.
                        """

                    # [변경 2] 질문에 특정 날짜(내일, 5월 5일 등)가 있는지 추가 파악
...
{query_ganji}
(위 데이터는 saju.db에서 조회한 확정된 사실입니다. 절대 재계산하지 말고 이 데이터를 근거로 답변하세요.)
...
1. **DB 데이터 절대 우선**: 위 [시스템 DB 확정 데이터]에 있는 간지(글자)를 사용하여 분석하십시오.
   - "만세력 계산 기능이 없다"는 말 금지. (이미 위에 데이터가 제공되었습니다)
   - 질문에 날짜가 없으면 위 '오늘' 데이터를 활용해 "참고로 오늘은 XX일이라 ~합니다"라고 덧붙여주십시오.

2. **구체적 추천**: "좋은 날짜를 전문가에게 물어보라"고 떠넘기지 마십시오.
   - 사용자가 막연하게 "언제가 좋아?"라고 물으면, 원국 분석(Part 1) 내용을 참고하여 
     "님에게는 金, 水가 용신이니 신(申), 유(酉), 해(亥), 자(子) 날이 좋습니다"라고 원론적인 가이드를 주십시오.
   - 그리고 "마침 오늘이 그 날입니다" 혹은 "오늘은 화(火) 기운이 강해 불리합니다"라고 오늘 데이터와 비교해주십시오.

3. **상담원 톤 유지**: 전문 용어를 쓰되 쉽게 풀어서 설명하십시오.
